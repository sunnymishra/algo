...
public class MakePalindromeByAdding{
	...
	*/
	public int makePalindrome2(String str){
		...
		int[][] cache=new int[str.length()][str.length()];
		int lcs = _lcs(str, reverseStr, str.length()-1, reverseStr.length()-1, cache);
		int edits=str.length()-lcs;
		return edits;
	}
	private int _lcs(String str1, String str2, int str1Idx, int str2Idx, int[][] cache){
		...
		if(str1.charAt(str1Idx)==str2.charAt(str2Idx)){
			...
			lcs= 1+ _lcs(str1, str2, str1Idx-1, str2Idx-1, cache);
		}else{
			int lcs1=_lcs(str1, str2, str1Idx-1, str2Idx, cache);
			int lcs2=_lcs(str1, str2, str1Idx, str2Idx-1, cache);
